...
```
minimum error rate when mtry = 15.

# tuning model: ntree
```{r}
rf<-randomForest(DEPRESSFLG~.,data=train_df,ntree=500,mtry=15,importance=TRUE)
plot(rf)
```
ntree = 100
...
# build final model
```{r}
rf<-randomForest(DEPRESSFLG~.,data=train_df,ntree=100,mtry=15,importance=TRUE)
```
